...
import { Activity, Brain, ShieldAlert, HeartHandshake, Eye } from "lucide-react";
...
export default function ChapterCard({ chapter }: ChapterCardProps) {
  ...
  const getAnatomicalIcon = (title: string) => {
    ...
    if (t.includes("cœur") || t.includes("cardio")) {
      return <Activity className="h-10 w-10 text-teal animate-pulse" style={{ animationDuration: "2.5s" }} />;
    }
    ...
    if (t.includes("neurologie") || t.includes("cerveau") || t.includes("ecg") === false) {
  ...
  const cardContent = (
    <div className={`relative flex flex-col h-full bg-white rounded-xl border border-teal/10 p-5 shadow-sm transition-all duration-300 ${chapter.isLocked ? "opacity-60 cursor-not-allowed" : "hover:shadow-md hover:border-teal/20 cursor-pointer"}`}>
      
      {/* Top Illustration Area */}
      <div className="relative flex h-24 items-center justify-center rounded-lg bg-surface/50 border border-teal/5 mb-4">
        {getAnatomicalIcon(chapter.title)}
        
        {chapter.isNew && (
          <span className="absolute top-2 right-2 rounded-full bg-teal px-2 py-0.5 text-[9px] font-bold text-white uppercase tracking-wider">
            NOUVEAU
          </span>
        )}
  ...
  return (
    <Link href={`/lessons/${chapter.moduleId}/${chapter.id}`} className="block h-full">
      {cardContent}
    </Link>
  );
}
